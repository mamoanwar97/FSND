...
def search_artists():
  ...
  search_term = request.form.get('search_term', '')
  search_result = Artist.query.filter(Artist.name.contains(search_term)).all()
  response = {
      "count": len(search_result),
      "data": search_result
  }
...
def shows():
  ...
  # TODO: replace with real venues data.
  #       num_shows should be aggregated based on number of upcoming shows per venue.
  all_show = Show.query.join(Artist).join(Venue).filter(Show.start_time>datetime.now()).all()
